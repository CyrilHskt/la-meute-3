...
const en: typeof fr = {
  ...
  presentation: {
    ...
    faq4Q: "What actually is a DAO, and what's Ethereum?",
    faq4A: "A DAO (\"decentralized autonomous organization\") is a bit like a digital house rulebook: rather than being written down in a spreadsheet someone could quietly edit, the association's decisions are recorded on a shared ledger nobody can tamper with afterward — every vote and every expense stays visible and checkable by everyone, forever. Ethereum is the computer network that hosts this ledger, and ETH is just the small amount of fuel that pays for the tiny cost of writing a line to it — a bit like a stamp to mail a letter.",
    faq5Q: "Why Base, and why not just stay on Ethereum directly?",
    faq5A: "Writing to Ethereum costs a little something each time (see the question above) — and depending on how busy the network is, that cost can sometimes get expensive, up to several tens of euros per vote. Base is an extension of Ethereum, built to make that writing nearly free, while keeping the exact same fuel (ETH) — not a currency invented for the occasion. We chose it because it makes access easier for a member who's never touched any of this before, not for some complicated technical reason.",
    faq6Q: "Wouldn't a regular association bank account have been enough?",
    faq6A: "A regular association bank account costs 3 to 7€ a month in account-keeping fees depending on the bank — a fixed amount the association has to anticipate and cover every year through membership dues, whether activity is high or nonexistent, and the bank can even close the account after prolonged inactivity. On the DAO, there's nothing to anticipate or pool in advance: each action (voting, applying, spending an expense) costs a few cents, paid directly by the member who performs it, right when they do. It's funded live, by usage — not through a collective reserve calculated ahead of the year's expected costs.",
